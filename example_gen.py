...
for item in example_data.items():
    ...
    if dataType == 'int':
        ...
        example_data[item[0]] = round(random.uniform(0, 7), 4) # Generate random float rounded to 4 decimals
    
    elif dataType == 'str':
        example_data[item[0]] = input("Input example string for <%s>: " % item[0]) # Prompt user to enter an example string, specifying the datapoint
        ...
        values = input("Input example values for list <%s>\n(Values separated by \',\'): " % item[0]).split(',')
        ...
        for i, value in enumerate(values):
            try:
                ...
                example_data[item[0]] = values # Runs if changing value to an int fails, indicating that it is most likely a string
